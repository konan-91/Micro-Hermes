...
A load balancer spreads incoming requests across a pool of servers so none is overwhelmed. This dissertation addresses that decision one level down: once traffic reaches a single machine, a Layer 7 (L7) load balancer, which processes application content rather than just packets, must decide which of its worker processes handles each new connection. Linux provides two mechanisms for this, epoll's exclusive wakeup and SO_REUSEPORT's stateless hash, both blind to how busy each worker is. Because L7 work varies wildly in cost, this blindness concentrates load on a few workers, inflates tail latency, and keeps routing traffic to hung workers. Hermes (Pan et al., 2025), a production system at Alibaba Cloud, closes this gap: workers publish their live status into shared memory, and an in-kernel eBPF program steers new connections towards available workers. Hermes is closed-source. Micro-Hermes is an open-source Rust implementation, built in two stages: a userspace simulation of the three-stage feedback loop, then a working version running a real in-kernel eBPF program over real sockets. Both are evaluated across the paper's four traffic regimes plus a fifth introduced here. The results support the paper's central claim: neither standard mechanism is safe in every regime, while Hermes routes around hung workers, balances connections an order of magnitude more evenly than epoll exclusive, and holds the best tail latency under overload. The working version also exposes a cost the simulation could not: under very high rates of cheap connections, the system calls publishing worker status outweigh the benefit.
...
All five objectives were met. O1, O2 and O4 were achieved in full in the simulation. O3 was achieved with two deviations from the paper's rankings, which the simulation attributed to its own limitations rather than to the architecture; completing O5 made it possible to test that attribution directly, and Section 8.6 reports the outcome, which confirms one explanation and refutes the other. O5 itself was the riskiest objective and was scoped as could-have from the outset (Chapter 3), because unlike O1-O4 it cannot be built or tested in ordinary application code: it needs a running Linux kernel, administrator privileges, and code accepted by the kernel's own safety checker. Completing it is what turns this project from a study of the architecture into a working implementation of it, and it is what makes the baseline comparison trustworthy, since the two mechanisms Hermes is measured against are no longer models written by the author but the kernel's own behaviour.

1.4 Report Structure

Chapter 2 gives the background needed to follow the rest of the dissertation and reviews related work together: what a load balancer is and why one is needed, the Layer 4 / Layer 7 distinction, the history of load balancing, the Linux mechanisms for distributing connections inside a single server and the research systems that have tried to improve on them, eBPF, and the Hermes architecture itself, closing with where Micro-Hermes sits relative to that body of work. Chapter 3 specifies the requirements the software had to meet, distinguishing those inherited directly from the Hermes paper from those the author added. Chapter 4 describes the development process, and Chapter 5 records ethical considerations. Chapter 6 presents the design, which both versions share, since the architecture being replicated is the same in each. Chapter 7 covers the two implementations, first the parts common to both, then the simulation and the eBPF version separately, since this is where they genuinely differ. Chapter 8 evaluates them: it establishes a common method, reports each version's results in its own section, and then compares the two directly, explaining where they agree, where they diverge, and why. Chapter 9 concludes with contributions, limitations, and the work that remains. Appendices provide the testing summary, a user manual for building and reproducing every result, and the ethics self-assessment.
...
The cause is the one anticipated in Section 6.4 and recorded in Section 7.5: publishing the candidate bitmap costs a system call, and the scheduler publishes at the end of every loop iteration. Case 1 at heavy load is 3,000 cheap connections per second, so the event loop turns over constantly and each turn pays that cost, while the actual work per connection is only a millisecond. The mechanism is paying full price for information the workload is too uniform to need, since with cheap, uniform requests no worker stays busy long enough for worker-awareness to be worth anything. This is not a defect in the port; it is the real cost of the design, made visible by running it for real. It is also consistent with what the Hermes authors themselves report: in their overhead breakdown the system calls that update the eBPF map are the single largest component under heavy load, larger than the scheduler and far larger than the in-kernel dispatcher (Pan et al., 2025). The simulation could not have surfaced this, because there the same publication step was one memory instruction.
...
What the simulation got wrong. Section 8.7 argued that the simulated epoll-exclusive baseline was flattered by an unrealistic advantage: when every worker was busy it consulted per-worker backlogs that the real mechanism does not possess, and the conclusion drawn was that the latency comparison was therefore "conservative towards Micro-Hermes", with the real gap likely larger in Micro-Hermes's favour. That prediction was wrong. With the modelling shortcut removed entirely, and the baseline now being the kernel's own wait-queue behaviour with genuinely no per-worker backlogs to inspect, real epoll exclusive still posts the lowest mean latency in Cases 2 and 4. The advantage was not an artefact of the model.

The reason, visible only once both mechanisms are real, is that waking whichever worker is idle is simply a very good heuristic for mean latency, and it needs no information at all to apply. Its cost is not latency but distribution: it achieves that mean by loading a small number of workers heavily, which is precisely what produces the concentration in Case 3, the burst penalty in Case 5, and the worst tail latency of the three policies under Case 2's overload. The honest conclusion is therefore narrower than the simulation's but better supported: Hermes does not beat epoll exclusive on average latency, and this project's earlier reasoning for expecting it to was mistaken. What Hermes provides is protection against the tail and against accumulated imbalance, which is what the architecture was designed for and what the production incidents motivating it actually involved.
...
The overall picture is that the architecture's value is real but conditional. It is worth paying for when per-connection work is expensive or variable, when workers can stall, or when connections are long-lived enough for imbalance to accumulate. It is not worth paying for when work is cheap and uniform, because the cost of continuously publishing status then exceeds the value of the information. The paper's own production data suggests this condition is usually met, since the two regimes that dominate Alibaba's traffic are long-lived connections and expensive processing (Section 2.8), which are exactly the two where Micro-Hermes performs well here.
...
The inference drawn from this at the time, that the comparison was conservative towards Micro-Hermes, has been tested directly and did not hold (Section 8.6). The factors identified were real, but they were not large enough to account for the gap, and the gap did not close when they were removed. This is recorded as a corrected conclusion rather than quietly dropped, because the reasoning was reasonable given the evidence then available and its failure is itself a result: it demonstrates concretely why the second version was worth building, and it is the strongest argument in this dissertation for not stopping at a simulation.
...
Fifth, a result the original paper does not report and the simulation could not have produced: a measured boundary on where the architecture is worth deploying. Because publishing worker status to the kernel costs a system call on every loop iteration, the mechanism becomes counterproductive under high volumes of cheap, uniform work, where it is roughly nine times slower than simply waking an idle worker. This is a genuine limit of the design rather than of this implementation, and it corroborates from the outside the paper's own finding that map-update system calls dominate its overhead budget under heavy load.
...
Two further limitations follow from the evaluation's scale and workload. It runs four workers on one machine with three trials per point, so the results establish orderings and trends rather than production performance figures. Its connections are also synthetic and uniform within each case, which is why Micro-Hermes matches rather than beats reuseport on balance in Case 3: a stateless hash only degrades when connection lifetimes vary widely, and here they do not (Section 8.7).
