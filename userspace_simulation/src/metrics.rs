//! CSV output for offline analysis. The ticks CSV has one row per worker
//! loop iteration (WST snapshot, queue depth, Algorithm 1 output) and
//! drives the balance-over-time plots. The conns CSV has one row per
//! completed connection and drives the latency comparisons.
//!
//! Each worker buffers rows locally and writes a private headerless shard
//! file when its loop ends, and the parent merges the shards after
//! waitpid. Deliberately not shared memory, a Vec's heap buffer isn't in
//! the shared region after fork() and std Mutexes are UB across processes
//! on macOS
...
/// One row per completed event, either a new connection's initial request
/// (kind "accept") or a Case 5 burst follow-up (kind "burst"). For burst
/// rows arrival_ns is when the burst fired, so latency_us measures how
/// long the follow-up waited behind its siblings
#[derive(Debug, Clone)]
pub struct ConnRow {
    pub conn_id: u64,
    pub worker_id: usize,
...
/// Merge headerless shard files into out_path, sorted numerically by the
/// first column. Returns the merged data lines for summarization
pub fn merge_shards(
    shard_paths: &[PathBuf],
    out_path: &str,
    header: &str,
...
pub fn summarize_conns(lines: &[String]) -> ConnSummary {
    ...
    for line in lines {
        ...
        // see conn_header, worker_id is field 1, latency_us is field 7
