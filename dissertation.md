...
This dissertation addresses two problems. First, the technical problem inherited from Hermes: kernel connection dispatch is blind to userspace load. epoll's exclusive wakeup mode prefers the most recently registered worker, which concentrates connections onto a few workers. SO_REUSEPORT's hash spreads connections evenly on average but keeps blindly dispatching to workers that are overloaded, hung, or crashed. L7 processing cost varies by orders of magnitude between connections and is unknowable at dispatch time, so per-worker load diverges even under a perfectly uniform hash. Only userspace knows how loaded a worker really is, and only the kernel decides where connections go; a solution must carry information across that boundary cheaply and safely.
...
Hardware appliances scale poorly in terms of cost and flexibility, and hyperscalers eventually replaced them with software load balancers running on commodity machines: Microsoft's Ananta scaled capacity by adding forwarding machines behind a replicated control plane (Patel et al., 2013), and Google's Maglev matched the raw performance of dedicated hardware from a single commodity server while introducing consistent hashing for backend selection (Eisenbud et al., 2016). Subsequent research refined the same design, removing per-connection state and hardening routing during pool changes (Olteanu et al., 2018; Barbette et al., 2020).
...
Later systems replaced the kernel's networking and scheduling stack entirely, correcting bad placements after the fact by stealing, preempting, or reassigning work that has already arrived, at microsecond timescales (Prekas et al., 2017; Kaffes et al., 2019; Ousterhout et al., 2019; Fried et al., 2020). This achieves stronger tail latency guarantees than any dispatch time policy can, but at a cost: these systems replace the operating system's I/O path, so applications must be rewritten against new interfaces and operators must run non-standard stacks.
...
Development began with a close reading of the paper, from which a design document was written recording every architectural commitment, which Chapter 3 is derived from. It also settled where corners could be cut, as where the paper specified something the implementation had to match it, and where it left something open the choice was recorded and justified. Therefore, the process was plan driven: the kernel/userspace split was decided before any code was written so the eBPF version could later replace components without restructuring the system (Section 6.1). Within that fixed architecture, implementation proceeded in three increments, each begun only once the previous ran end to end.

The first increment was a stripped-down simulator: four simulated cores running simple, non-atomic versions of the three dispatch algorithms, with no shared memory or metrics collection, which surfaced the feedback loop's basic dynamics early and cheaply. The second built this skeleton out to the paper's full specification one component at a time, with unit tests written for each component before moving on; the suite turns the paper's algorithms and fallback rules into executable checks. Implementation problems still surfaced: Section 7.5 records five, reported there rather than quietly fixed. The third increment replaced the simulated kernel with the real one, deliberately last because it alone requires a Linux kernel, administrator privileges, and code accepted by the kernel's safety checker. Porting only once the architecture was settled and the evaluation harness worked meant the port was judged against existing results rather than debugged alongside the design, and the components designed to carry over unchanged did so (Section 7.3).
...
The dispatcher implements Hermes's Algorithm 2. For each new connection it reads the candidate bitmap from M_Sel, counts the set bits, and, if more than one candidate survives, scales the connection's hash into the candidate count using the Linux kernel's reciprocal_scale function (reimplemented exactly) and selects the Nth set bit of the bitmap. If one or zero candidates survive, it falls back to plain reuseport hashing across all workers, matching the paper's fallback rule (Section 2.8).
...
One consequence of this design is hidden in the simulation but significant in the eBPF version - publishing the bitmap into a table the kernel owns requires a system call on every loop iteration, a cost that Section 8.3 measures and Section 8.6 explains.
...
Fifth, the cost of publishing status, which appeared not as a defect but as a result. Writing the candidate bitmap is a single memory store in the simulation and a system call in the eBPF version, paid on every loop iteration, and under Case 1 at heavy load it becomes the dominant cost of the entire mechanism and inverts the ranking (Sections 8.3 and 8.6). No adjustment was made in response; the result is reported as measured.
