...
//!     `SelMap` (store/load one u64) deliberately mirrors a single-element
//!     `BPF_MAP_TYPE_ARRAY` so only the internals of `SelMap` change
//!     (atomic store -> bpf map update syscall).
//!   - `queues`: Phase-1 only, stands in for the kernel's per-socket accept
//!     queues. Replaced by real SO_REUSEPORT sockets.
