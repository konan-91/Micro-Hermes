//! Phase 1 userspace simulation of the Hermes load balancer. The parent
//! stands in for the kernel, pacing arrivals and running the dispatch
//! mechanism under test. The children are forked workers running the
//! instrumented event loop (Fig. 9). All cross-process state lives in one
//! shared mmap region, see shm.rs.
//!
//! Configured through environment variables, POLICY, WORKLOAD_CASE, LOAD,
//! METRICS_PATH, CONNS_PATH, VERBOSE and SEED
...
fn main() {
    ...
    // parent generates traffic, then reaps children
