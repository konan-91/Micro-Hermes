...
A load balancer spreads incoming requests across a pool of servers so that none of them is overwhelmed. This dissertation addresses that decision one level down: once traffic reaches a single machine, a Layer 7 (L7) load balancer, which processes application content rather than just packets, must decide which of its own worker processes handles each new connection. Linux provides two mechanisms for this, epoll's exclusive wakeup and SO_REUSEPORT's stateless hash, both blind to how busy each worker is. Because L7 work varies wildly in cost, this blindness concentrates load on a few workers, inflates tail latency, and keeps routing traffic to hung workers. Hermes (Pan et al., 2025), a production system at Alibaba Cloud, closes this gap: workers publish their live status into shared memory, and an in-kernel eBPF program steers new connections towards available workers. Hermes is closed-source. This dissertation presents Micro-Hermes, an open-source Rust implementation, built here as a complete userspace simulation of Hermes's three-stage feedback loop using real forked processes and lock-free shared memory. It is evaluated against models of both Linux mechanisms across the paper's four traffic regimes, plus a fifth scenario introduced here. The results reproduce the paper's central claim: neither standard mechanism is safe in every regime, while Hermes detects and routes around hung workers within a bounded time, balances long-lived connections an order of magnitude more evenly than epoll exclusive, and remains competitive everywhere. Deviations from the paper's rankings trace to documented limits of this simulation, not the Hermes architecture itself.
...
6.5 Alternative Scheduling Algorithms

7. Implementation

7.1 Rust Toolchain and eBPF Integration

7.2 Shared Memory Communication

7.3 Simulating the Kernel Dispatch Path

7.4 Engineering Challenges

8. Evaluation and Critical Appraisal

8.1 Evaluation Framework and Methodology

8.2 Benchmarks: Micro-Hermes vs reuseport vs epoll_exclusive

8.3 Worker Hang Prevention Results

8.4 Scheduler Filter Behaviour
...
9.3 Objective O5: Scope and Reflection
...
The aim of the project is a faithful, open-source implementation of the Hermes architecture, pursued through five objectives in order of importance. This dissertation implements and evaluates the complete three-stage feedback loop as a userspace simulation, with the kernel pieces replaced by userspace stand-ins designed to be swapped for real eBPF code later, under O5; objectives O1-O4 define that scope, and O5 extends beyond it:
...
O5. (If possible) Begin the port of the dispatch path to a real eBPF program attached via SO_ATTACH_REUSEPORT_EBPF using the Aya toolchain.

Objectives O1, O2, and O4 were achieved in full. O3 was achieved with two explained deviations from the paper's rankings, both attributed to a specific limitation of the userspace simulation (Section 8.5) rather than to the architecture itself. O5 was not completed: unlike O1-O4, which are all buildable and testable entirely in userspace, it requires a real Linux kernel, root privileges, and code that passes the eBPF verifier, a materially larger and riskier undertaking, which is why O1-O4 were prioritised as must-have and O5 as could-have from the outset (Chapter 3). The reasoning behind that scoping decision, and what completing O5 would involve, is discussed in the concluding reflection on that objective (Section 9.3). The codebase is nonetheless structured so that O5, when undertaken, replaces components rather than restructuring the system (Section 6.1).

1.4 Report Structure

Chapter 2 gives the background needed to follow the rest of the dissertation and reviews related work together: what a load balancer is and why one is needed, the Layer 4 / Layer 7 distinction, the history of load balancing, the Linux mechanisms for distributing connections inside a single server and the research systems that have tried to improve on them, eBPF, and the Hermes architecture itself, closing with where Micro-Hermes sits relative to that body of work. Chapter 3 specifies the requirements the software had to meet, distinguishing those inherited directly from the Hermes paper from those the author added. Chapter 4 describes the development process, and Chapter 5 records ethical considerations. Chapters 6 and 7 present the design and implementation of the simulation, Chapter 8 evaluates it against the two baseline mechanisms and against the paper's published behaviour, and Chapter 9 concludes with contributions, limitations, and a reflection on the one objective not completed. Appendices provide the testing summary, a user manual for building and reproducing every result, and the ethics self-assessment.
...
The requirements follow from the project's nature as a replication. The system described in Pan et al. (2025) is the specification, so functional requirements FR1-FR7 and non-functional requirements NFR1-NFR2 are derived directly from the paper's architecture and implement objectives O1-O3 (Section 1.3): they exist because the paper's design demands them, not because the author chose them. FR8 and NFR3-NFR5 are the author's own additions, made to support evaluation and reproducibility rather than to replicate the architecture itself, and each is justified individually below; FR8 implements O4. FR9 implements O5, the eBPF port, and is scoped as could-have because completing it within the available time was judged materially riskier than O1-O4, for reasons discussed in the concluding reflection on that objective. Requirements are prioritised using MoSCoW (must/should/could).
...
Because the requirements were fixed up front, the overall process was plan driven at the level of architecture: the split between kernel-facing and userspace components was decided before any code was written, specifically so that objective O5's eBPF port can later replace components without restructuring the system (Section 6.1). Within that fixed architecture, however, implementation itself was iterative rather than waterfall, and deliberately so. Rather than building the full specification in one pass, the first working version was a stripped-down preliminary simulator: four simulated cores running simple, non-atomic versions of the Hermes, LIFO, and reuseport dispatch algorithms, with no dispatcher component, no lock-free shared memory, and no metrics collection at all. Getting this skeleton running end to end first, before any of the harder concurrency engineering was attempted, surfaced the closed feedback loop's basic dynamics and the workload generator's pacing behaviour early and cheaply. The full system was then fleshed out to the paper's specification in stages on top of that skeleton, one component at a time, shared memory first, then the WST, the scheduler, the dispatcher, the worker loop, the workload generator, and finally the benchmark harness, with unit tests written for each component before moving to the next. A fixed specification does not stop implementation problems from surfacing once real code exists (Section 7.4 records three of them). The test suite turns the paper's algorithms into executable checks: hang detection and θ behaviour for Algorithm 1, reciprocal_scale ranges, Nth-set-bit selection and the single or zero candidate fallback for Algorithm 2, the LIFO wait queue model, and the lock-free queue's wrap-around and overflow behaviour.

Testing the system as a whole, rather than its parts, was benchmark-driven. The rankings the paper expects in each traffic regime were written down as explicit validation targets before the benchmarks were run, and the analysis pipeline checks them automatically and prints a pass or fail verdict for each (Section 8.1). Git was used throughout, hosted on GitHub, with the analysis notebook, benchmark scripts and results all kept in the repository so that the whole evaluation can be reproduced from a clean checkout. Three deviations found during development (the metrics plumbing across fork(), the phase-locked timer artefact in the LIFO baseline, and the batch-size compensation) are reported in Section 7.4 rather than being quietly fixed and left undocumented.
...
Micro-Hermes's architecture is split deliberately along the kernel/userspace boundary. Every element of the original system that lives in userspace in the real system too, the Worker Status Table, the per-worker scheduler, the connection dispatcher's logic, and the shared-memory communication between them, is implemented faithfully here; the two elements that require a Linux kernel, the eBPF dispatch program and real sockets, are replaced by userspace stand-ins with equivalent interfaces. This split exists so that objective O5, porting the dispatch path to a real eBPF program attached via SO_ATTACH_REUSEPORT_EBPF, can later swap those stand-ins out without restructuring the rest of the system; O5 was not completed within this project's timeline, for reasons discussed in the concluding reflection on that objective, but the component boundary is designed to make that later port mechanical rather than a rewrite.
...
Although this simulation executes the logic in the parent process rather than in the kernel, it is written under eBPF's constraints (no heap allocation, statically bounded loops) so that porting it to a real eBPF program under objective O5 is straightforward. M_Sel itself is modelled as a single shared atomic 64-bit integer, mirroring the atomic read/write guarantee that BPF_MAP_TYPE_ARRAY provides in the real system. Only the internals of this component would need to change to complete that port.

6.4 Userspace Scheduler

Every worker runs Hermes's Algorithm 1 at the end of each event-loop iteration: there is no dedicated scheduler process, exactly as in the original design. The reason is the one established in Section 2.5: a dedicated dispatcher or scheduler process on the connection path becomes a bottleneck at high connection rates and a single point of failure, whereas embedding the scheduler in every worker means scheduling continues as long as *any* worker is alive — a property the evaluation observes directly when a stalled worker is excluded by its peers' schedulers (Section 8.3). The scheduler snapshots the WST and applies three cascading filters in the paper's fixed priority order. The time filter removes workers whose last loop entry is older than a hang threshold (200 ms in this implementation, the paper leaves the constant implementation-defined). The connection-count filter and the pending-event filter each remove workers whose metric exceeds the candidate average plus an offset theta, which prevents the candidate set from collapsing when workers are near-uniformly loaded. Following the paper's empirical finding that theta/avg = 0.5 is optimal, theta is computed as half the candidate average, with a small floor so the filter remains permissive at cold start. The surviving set is encoded as a bitmap and written to M_Sel with a single atomic store.
...
Building the preliminary simulator described in Chapter 4 before the full specification paid off directly here: because the closed feedback loop and the workload generator's pacing were already validated against a trivial, non-atomic version of the system, the three issues below could be isolated to the concurrency and fidelity engineering added afterward, rather than being tangled up with basic correctness questions about the loop itself.
...
Caption: 99th-percentile connection latency per policy (bars: mean of per-trial p99; error bars: min–max across trials; note the independent y-scales). Under load, reuseport's p99 exceeds Micro-Hermes's — by roughly 30% in Case 2 and 2.2x in Case 4. The LIFO model's low latency in those cases is a simulation boundary discussed in Section 8.5, and Figure 8 shows the liability that accompanies it.]
...
Finally, the paper's CPU overhead figures (0.674% to 2.436% total, across counters, scheduler, syscalls and the eBPF dispatcher) cannot be meaningfully checked here, since the simulation's "processing" is just sleeping rather than real work. Overhead measurement is deferred to objective O5, where flamegraph profiling of the real, instrumented event loop becomes possible.
...
First, the implementation itself (objectives O1 and O2): to the author's knowledge the first open-source implementation of the Hermes architecture — the shared-memory Worker Status Table, the per-worker cascading-filter scheduler, and the hash-among-candidates dispatcher — written in approximately 1,600 lines of dependency-light Rust, with the paper's lock-free concurrency design preserved exactly and the kernel-facing logic already written within eBPF's constraints for objective O5's later port.

Second, a simulation methodology that keeps the architecture honest (supporting O1 and O3): real forked processes communicating through genuinely shared memory, a closed feedback loop in which scheduling decisions alone shape the measured load distribution, and baselines that model the kernel's actual mechanisms (including epoll exclusive's static LIFO wait queue) rather than caricatures of them.

Third, an independent reproduction of the paper's qualitative claims (O3): across the four published traffic regimes, each swept over three offered-load levels as in the paper's evaluation, the simulation reproduces the paper's rankings wherever the simulation is competent to test them — Hermes beats the stateless hash decisively when workers are busy or hung, with a gap that widens monotonically with load and extends to throughput once offered load nears capacity; balances long-lived connections an order of magnitude more evenly than exclusive wakeup; detects an injected worker stall within a bounded window through the other workers' schedulers; and, uniquely among the three mechanisms, degrades gracefully in every regime. The two deviations from the paper's rankings were traced to identified boundaries of a userspace simulation rather than to the architecture.

Fourth, an extension of the paper's evaluation (O4): the synchronised-burst scenario (Case 5) converts the paper's argument for the connection-count metric from a rationale into a measurement, showing the exclusive model's apparently strong latency coexisting with a 3-4x burst-latency liability created by its connection hoarding.

9.2 Limitations

The central limitation is that this evaluation validates objectives O1-O4, the architecture, not O5, the kernel mechanism: no eBPF program runs, no real sockets exist, and the paper's CPU overhead profile cannot be checked against work that consists of sleeps. Within the simulation, the epoll-exclusive baseline is structurally better-informed than the mechanism it models (Section 8.5), so its winning latency in Cases 2 and 4 must not be read as a property of real epoll exclusive; the comparison is conservative against Micro-Hermes. The evaluation scale is deliberately small — four workers on one machine, synthetic traffic with identical cost distributions per case, three trials per cell — so, although each case is swept across three offered-load levels, the results establish qualitative orderings and load trends, not production performance numbers. Finally, two of Hermes's production subsystems are out of scope entirely: the TCP-RST mitigation for connections already pinned to a hung worker, and the cluster-level anomaly detection and progressive scaling that handle node-wide overload.

9.3 Objective O5: Scope and Reflection

Objective O5, porting the dispatch path to a real eBPF program, was scoped as could-have in Chapter 3 and was not completed within this project's timeline. The reason is not that it was judged less important than O1-O4, but that it is a materially different, and materially riskier, kind of work. O1-O4 could be designed, built, tested, and benchmarked entirely in userspace, on any machine, with failures that are ordinary software bugs caught by unit tests and benchmark predicates. O5 requires a real Linux kernel with root or CAP_BPF privileges, real SO_REUSEPORT sockets, and a program that must additionally satisfy the eBPF verifier, a static analysis pass with no userspace equivalent, which can reject a correct-looking program outright and whose failure modes are unfamiliar and slower to debug by comparison. Within a single-developer, 11-week timeline, attempting O5 alongside O1-O4 risked delivering neither rigorously; the requirements process (Chapter 3) reflected this by prioritising O1-O4 as must-have and O5 as could-have from the outset, and the time that would otherwise have gone to a rushed, thinly-tested kernel port went instead to the iterative build-up described in Chapter 4, the fifth evaluation scenario (O4), and the testing depth reported in Appendix A.

Completing O5 means replacing the simulated dispatcher with a real eBPF program attached via SO_ATTACH_REUSEPORT_EBPF (written with Aya), giving each worker a real SO_REUSEPORT listening socket registered in a BPF_MAP_TYPE_REUSEPORT_SOCKARRAY, and driving the system with a real traffic generator over real epoll. The WST, scheduler, and worker instrumentation carry over unchanged, since they live in userspace in the real system too, which is precisely what the architecture split described in Section 6.1 was designed to make possible. Completing it would settle the two questions this simulation cannot: the true latency ordering against real epoll exclusive, and the CPU overhead profile, measurable by flamegraph against the paper's published 0.674%-2.436% envelope.

Beyond O5, several extensions follow naturally from the objectives already met. Scaling the worker count towards production-like values (the paper's deployments run tens of workers over tens of thousands of ports) would test the bitmap encoding and filter behaviour at realistic width. Implementing the paper's exception-handling path — proactive TCP RST of connections pinned to a hung worker, forcing clients to reconnect through the healthy dispatch path — would complete the reliability story begun by O1's hang detection. The scheduler itself invites experimentation the closed system cannot offer: alternative filter orderings, dynamic θ tuning, or entirely different candidate-selection policies can be evaluated against the same benchmark harness built for O3. Finally, the production pitfalls Alibaba reports downstream of even balancing — per-worker round-robin restart skew and reduced backend connection reuse — suggest a companion investigation into worker-shared backend connection pools.
