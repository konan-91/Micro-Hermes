...
The related work above reveals a clear gap. Layer 4 eBPF load balancing is mature and well studied (Section 3.2). Intra-server scheduling research achieves strong guarantees, but only by replacing the operating system's stack (Section 3.3). Application aware dispatch that works with stock Linux, preventing some worker processes from becoming overloaded while others sit idle, exists only in proprietary industry systems like Hermes. No open-source, reproducible implementation exists to validate or extend this approach. 
...
The requirements follow from the project's nature as a replication. The system in the original paper is the specification, and so sticking faithfully to the original design is the primary requirement. Requirements are prioritised using MoSCoW (must/should/could). Functional requirements FR1-FR7 define Phase 1's faithful replication of Hermes, FR8 extends the evaluation of the system, and FR9 defines Phase 2.
...
FR5 (must). Implement two baseline dispatch policies on the same infrastructure for comparison: SO_REUSEPORT's stateless hash, and a model of epoll exclusive's static LIFO wait queue wakeup.
...
FR8 (should). Implement a fifth scenario, beyond the paper's four: a synchronised burst of follow up requests across accumulated long lived connections. Used to measure the cost of connection concentration/accumulation.
...
NFR1 (must). No locks anywhere, including the WST, the candidate map, and the accept queues. Each field must be individually safe to read, but the three fields can be slightly out of sync with each other, exactly as the paper allows.

NFR2 (must). The kernel code must be written within eBPF's constraints (no heap allocation, statically bounded loops), so the Phase 2 port is a mechanical swap rather than a rewrite.
...
NFR4 (should). Where the simulation has to deviate from the real system (for example the simulated work), the deviation must be documented and its effect on the conclusions discussed (Sections 8.4, 9.5).

NFR5 (should). Phase 1 should use only the Rust standard library plus libc (for mmap, fork, waitpid).
